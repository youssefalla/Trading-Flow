...
export function MasterSidebar({ profile, onAvatarChange }: Props) {
  ...
  return (
    <aside className="w-64 hidden md:flex flex-col shrink-0 tf-sidebar" style={{ padding: '24px 16px' }}>
      ...
      <Link href="/" className="flex items-center gap-2 px-2 mb-6">
        ...
        <span style={{ fontFamily: SYS, fontWeight: 700, fontSize: '1.1rem', color: 'var(--tf-text)', letterSpacing: '-0.02em' }}>
      ...
      <nav className="flex-1 space-y-5">
        {navGroups.map(group => (
          <div key={group.label}>
            <div className="px-3 mb-2 text-[10px] font-semibold tracking-widest" style={{ color: 'var(--tf-subtle)' }}>
              {group.label}
            </div>
      ...
      {/* Profile card at bottom — Fintrixity style */}
      <div className="mt-4 rounded-2xl p-4" style={{ background: 'rgba(201,168,76,.07)', border: '1px solid rgba(201,168,76,.15)' }}>
        <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => avatarInputRef.current?.click()}
            className="relative w-10 h-10 rounded-full shrink-0 overflow-hidden group"
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: 'rgba(0,0,0,0.5)' }}>
              {uploading ? <span className="text-white text-[9px]">…</span> : <Camera size={13} color="white" />}
            </div>
          </button>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold truncate" style={{ color: 'var(--tf-text)', fontFamily: SYS }}>{profile?.full_name ?? '—'}</div>
            <div className="text-[11px]" style={{ color: '#C9A84C' }}>
              {profile?.role === 'coach' ? 'Coach ✓' : 'Master Trader ✓'}
            </div>
          </div>
          <ThemeToggle />
        </div>
        {profile?.city && (
          <div className="text-[11px] mt-2 px-1" style={{ color: 'var(--tf-subtle)' }}>{profile.city}</div>
        )}
      </div>
    </aside>
  )
